---
title: "MITD - healthy vs ARNt-Phe"
output:
  html_document:
    df_print: paged
    code_folding: hide
...
pbmc.libs <- subset(pbmc.cells, subset = orig.ident %in% c("MITD1_l1", "MITD1_l2"))
pbmc.libs <- subset(pbmc.libs, subset = Donorid %in% c("D03", "H03"))
...
# individual clusters
 saveRDS(pbmc.libs, "ARNtPhe_vs_healthy.all.processing.uptoUMAP_l1_l4_specific_donors.rds")
```

```{r}
pbmc.libs <- readRDS("ARNtPhe_vs_healthy.all.processing.uptoUMAP_l1_l4_specific_donors.rds")
```
...
#ggsave("ARNtPhe_vs_healthy_umap_celltypes_newnorm.png", dpi = 300, width = 30, height = 20, units = "cm")

DimPlot(pbmc.libs, group.by = "predicted.celltype.l2",reduction = "umap", label = "F")
ggsave("ARNt-Phe_vs_healthy_umap_celltypes_nosplit.pdf", dpi = 300, width = 30, height = 20, units = "cm")

DimPlot(pbmc.libs, reduction = "umap", label = "T", label.size = 4, repel = T, split.by = "Group", group.by = "predicted.celltype.l2") + NoLegend() + ggtitle("Healthy vs ARNt-Phe celltypes")
ggsave("ARNt-Phe_vs_healthy_umap_celltypes.pdf", dpi = 300, width = 30, height = 20, units = "cm")


DimPlot(pbmc.libs, reduction = "umap", label = F, split.by = "Group", group.by = "predicted.celltype.l2") + ggtitle("Healthy vs ARNt-Phe celltypes")
ggsave("ARNt-Phe_vs_healthy_umap_celltypes_legends.pdf", dpi = 300, width = 35, height = 20, units = "cm")

DimPlot(pbmc.libs, reduction = "umap", label = "T", label.size = 4, repel = T, split.by = "Group", group.by = "predicted.celltype.l1") + NoLegend() + ggtitle("Healthy vs ARNt-Phe celltypes")
ggsave("ARNt-Phe_vs_healthy_umap_celltypes_upperlevel_annotation.pdf", dpi = 300, width = 30, height = 20, units = "cm")


DimPlot(pbmc.libs, reduction = "umap", label = F, split.by = "Group", group.by = "predicted.celltype.l1") + ggtitle("Healthy vs ARNt-Phe celltypes")
ggsave("ARNt-Phe_vs_healthy_umap_celltypes_legends_upperlevel_annotation.pdf", dpi = 300, width = 35, height = 20, units = "cm")

pbmc_nona <- pbmc.libs[,!colnames(pbmc.libs) %in% remove_cells]

DimPlot(pbmc_nona, reduction = "umap", label = F, split.by = "Group", group.by = "manual.celltype.l1.l2") + ggtitle("Healthy vs ARNt-Phe celltypes")
ggsave("ARNt-Phe_vs_healthy_umap_celltypes_legends_manual_annotation.pdf", dpi = 300, width = 35, height = 20, units = "cm")

DimPlot(pbmc_nona, reduction = "umap", label = F, group.by = "manual.celltype.l1.l2") + ggtitle("Healthy vs ARNt-Phe celltypes")
ggsave("ARNt-Phe_vs_healthy_umap_celltypes_nosplit_manual_annotation.pdf", dpi = 300, width = 30, height = 20, units = "cm")
...
pseudobulk <- function(celltypes)
{
  for(i in 1:length(celltypes))
  {
    ...
    Idents(data) <- "Group"
    de_markers <- FindMarkers(data, ident.1 = "Healthy", ident.2 = "ARNt-Phe", test.use = "MAST")
...
expr_mat <- pbmc.libs.cd14[["RNA"]]@counts %>% 
  as.data.frame() %>% 
  rownames_to_column("Symbol") %>% 
  filter(Symbol %in% sig_res_cd14$Symbol) %>% 
  left_join(data.frame(Symbol = sig_res_cd14$Symbol), .) %>% 
...
expr_mat_ <- expr_mat %>% 
  ...
  mutate(status = case_when(
    Symbol %in% mitogenes ~ "mito",
    .default = ""
  )) %>% 
  mutate(colors = case_when(
    Symbol %in% mitogenes ~ "red",
    .default = "black"
  )) %>% 
  column_to_rownames("Symbol")
...
pdf("ARNt-Phe_vs_healthy_heatmap_CD14.pdf")
pheatmap(expr_mat,
         color = heat_colors,
         cluster_rows = F,
         cluster_cols = T,
...
top10 <- cd14[[1]] %>% 
  ...
  arrange(avg_log2FC)
pdf("ARNt-Phe_vs_healthy_heatmap_CD14.pdf")
DoHeatmap(cd14.libs, features = top10$Symbol, size = 4, angle = 0, group.by = "Group") +     scale_fill_gradientn(colors = rev(brewer.pal(11, "PuOr"))) 
dev.off()
...
top10 <- nk[[1]] %>% 
  ...
  # filter(Symbol %in% mitogenes)
  arrange(avg_log2FC)
pdf("ARNt-Phe_vs_healthy_heatmap_NK.pdf")
DoHeatmap(nk.libs, features = top10$Symbol, size = 4, angle = 0, group.by = "Group") +     scale_fill_gradientn(colors = rev(brewer.pal(11, "PuOr"))) 
dev.off()
...
ggsave("ARNt-Phe_vs_healthy_ModuleScore_Mitocarta_CD14_CD16_NK.pdf", width = 8, height = 5)



ModScore_healthy = subset(ModScore, subset = Group == "Healthy")
FeaturePlot(ModScore_healthy,
            features = 'Cluster1', label = TRUE, repel = TRUE) +
            scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdBu"))) +
  ggtitle("ModScore_healthy")

ModScore_mtTL1 = subset(ModScore, subset = Group == "ARNt-Phe")
FeaturePlot(ModScore_mtTL1,
            features = 'Cluster1', label = TRUE, repel = TRUE) +
            scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdBu"))) +
  ggtitle("ModScore_ARNt-Phe")
